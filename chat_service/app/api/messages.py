...
async def create_message(
    ...
    await event_dispatcher.dispatch(
        MessageCreated(
            ...
            user=UserInfo(id=current_user.id, username=current_user.username),
            is_deleted=db_message.is_deleted,
        )
    )

    unread_counts = await chat_interactor.get_unread_counts_for_chat_members(
        db_message.chat_id, current_user.id
    )
    for user_id, unread_count in unread_counts.items():
        await event_dispatcher.dispatch(
            UnreadCountUpdated(
                chat_id=db_message.chat_id, user_id=user_id, unread_count=unread_count
            )
        )
...
async def update_message(
    ...
    if not updated_message:
        raise HTTPException(
            status_code=404,
            detail="Message not found or you're not authorized to update it",
        )
    ...
    await event_dispatcher.dispatch(
        MessageUpdated(
            ...
            updated_at=updated_message.updated_at or updated_message.created_at,
            user=UserInfo(id=current_user.id, username=current_user.username),
...
async def delete_message(
    ...
    if not deleted_message:
        raise HTTPException(
            status_code=404,
            detail="Message not found or you're not authorized to delete it",
        )
...
async def update_message_status(
    ...
    unread_count = await chat_interactor.get_unread_messages_count(
        updated_message.chat_id, current_user.id
    )
