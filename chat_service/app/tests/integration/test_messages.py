...
async def test_create_message(
    client: AsyncClient, auth_header, mock_redis, test_user, test_chat
):
...
async def test_delete_message(client: AsyncClient, auth_header, test_chat):
    ...
    delete_response = await client.delete(
        f"/api/v1/messages/{message_id}", headers=auth_header
    )
    assert delete_response.status_code == 204

    # Verify deletion by fetching messages and checking the content
    messages_response = await client.get(
        f"/api/v1/messages/{test_chat.id}", headers=auth_header
    )
...
async def test_get_messages_with_pagination(
    ...
    response1 = await client.get(
        f"/api/v1/messages/{test_chat.id}?skip=0&limit=10", headers=auth_header
    )
    ...
    response2 = await client.get(
        f"/api/v1/messages/{test_chat.id}?skip=10&limit=10", headers=auth_header
    )
...
async def test_update_message_not_found(client: AsyncClient, auth_header):
    response = await client.put(
        "/api/v1/messages/99999", headers=auth_header, json={"content": "Updated"}
    )
    assert response.status_code == 404
...
async def test_update_message_status_not_found(client: AsyncClient, auth_header):
    response = await client.put(
        "/api/v1/messages/99999/status", headers=auth_header, json={"is_read": True}
    )
    assert response.status_code == 404
...
async def test_create_message_unauthorized(client: AsyncClient, test_chat):
    response = await client.post(
        "/api/v1/messages/", json={"content": "Hello", "chat_id": test_chat.id}
    )
...
async def test_update_message_unauthorized(client: AsyncClient, test_chat):
    response = await client.put("/api/v1/messages/99999", json={"content": "Updated"})
    assert response.status_code == 401
...
async def test_update_message_status_unauthorized(client: AsyncClient):
    response = await client.put("/api/v1/messages/99999/status", json={"is_read": True})
    assert response.status_code == 401


async def test_get_messages_with_content_filter(
    client: AsyncClient, auth_header, test_chat
):
    ...
    response = await client.get(
        f"/api/v1/messages/{test_chat.id}?content=Hello", headers=auth_header
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
async def test_update_message_same_user_only(
    client: AsyncClient, auth_header, test_chat, test_user2
):
...
async def test_delete_message_same_user_only(
    client: AsyncClient, auth_header, test_chat, test_user2
):
    # Create message with first user
    create_response = await client.post(
    ...
    response = await client.delete(
        f"/api/v1/messages/{message_id}", headers=user2_auth_header
    )
    assert response.status_code == 404
