...
async def test_get_chats(client: AsyncClient, auth_header):
    ...
    await client.post(
        "/api/v1/chats/", headers=auth_header, json={"name": "Chat 1", "member_ids": []}
    )
    await client.post(
        "/api/v1/chats/", headers=auth_header, json={"name": "Chat 2", "member_ids": []}
    )
...
async def test_get_chats_with_name_filter(client: AsyncClient, auth_header):
    await client.post(
        "/api/v1/chats/",
        headers=auth_header,
        json={"name": "Alpha Chat", "member_ids": []},
    )
    await client.post(
        "/api/v1/chats/",
        headers=auth_header,
        json={"name": "Beta Chat", "member_ids": []},
    )
...
async def test_start_chat_with_nonexistent_user(client: AsyncClient, auth_header):
    response = await client.post(
        "/api/v1/chats/start", headers=auth_header, json={"other_user_id": 99999}
    )
    assert response.status_code == 404
...
async def test_update_chat(client: AsyncClient, auth_header):
    ...
    update_response = await client.put(
        f"/api/v1/chats/{chat_id}", headers=auth_header, json={"name": "Updated Chat"}
    )
...
async def test_update_nonexistent_chat(client: AsyncClient, auth_header):
    response = await client.put(
        "/api/v1/chats/99999", headers=auth_header, json={"name": "Updated Chat"}
    )
    assert response.status_code == 404
...
async def test_delete_chat(client: AsyncClient, auth_header):
    ...
    delete_response = await client.delete(
        f"/api/v1/chats/{chat_id}", headers=auth_header
    )
...
async def test_add_nonexistent_member(client: AsyncClient, auth_header, test_user):
    ...
    response = await client.post(
        f"/api/v1/chats/{chat_id}/members", headers=auth_header, json={"user_id": 99999}
    )
    assert response.status_code == 404
...
async def test_remove_chat_member(
    ...
    response = await client.delete(
        f"/api/v1/chats/{chat_id}/members/{test_user2.id}", headers=auth_header
    )
...
async def test_remove_nonexistent_member(client: AsyncClient, auth_header, test_user):
    ...
    response = await client.delete(
        f"/api/v1/chats/{chat_id}/members/99999", headers=auth_header
    )
    assert response.status_code == 404
...
async def test_get_unread_messages_count(client: AsyncClient, auth_header, test_user):
    ...
    response = await client.get(
        f"/api/v1/chats/{chat_id}/unread_count", headers=auth_header
    )
    assert response.status_code == 200
...
async def test_create_chat_with_duplicate_members(
    ...
    response = await client.post(
        ...
        headers=auth_header,
        json={
            "name": "Duplicate Members Chat",
            "member_ids": [test_user.id, test_user.id],
        },
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_get_chats_with_complex_name_filter(client: AsyncClient, auth_header):
    await client.post(
        "/api/v1/chats/",
        headers=auth_header,
        json={"name": "Alpha Beta Chat", "member_ids": []},
    )
    await client.post(
        "/api/v1/chats/",
        headers=auth_header,
        json={"name": "Beta Gamma Chat", "member_ids": []},
    )
    await client.post(
        "/api/v1/chats/",
        headers=auth_header,
        json={"name": "Gamma Alpha Chat", "member_ids": []},
    )
...
async def test_start_chat_with_self(client: AsyncClient, auth_header, test_user):
    response = await client.post(
        "/api/v1/chats/start", headers=auth_header, json={"other_user_id": test_user.id}
    )
    assert response.status_code == 404
    assert "User not found" in response.json()["detail"]
...
async def test_create_chat_and_add_message(
    ...
    test_user2_auth_header = {
        "Authorization": f"Bearer {login_response.json()['access_token']}"
    }

    # Check unread count for test_user2
    unread_response = await client.get(
        f"/api/v1/chats/{chat_id}/unread_count", headers=test_user2_auth_header
    )
...
async def test_unauthorized_requests(client: AsyncClient, test_chat):
    # Create chat
    response = await client.post(
        "/api/v1/chats/", json={"name": "Test Chat", "member_ids": []}
    )
    ...
    # Start chat
    response = await client.post("/api/v1/chats/start", json={"other_user_id": 1})
    assert response.status_code == 401
    ...
    # Update chat
    response = await client.put(
        f"/api/v1/chats/{test_chat.id}", json={"name": "Updated Chat"}
    )
    assert response.status_code == 401
    ...
    # Add member
    response = await client.post(
        f"/api/v1/chats/{test_chat.id}/members", json={"user_id": 1}
    )
    assert response.status_code == 401
...
async def test_pagination_edge_cases(client: AsyncClient, auth_header):
    ...
    response = await client.get(
        "/api/v1/chats/?skip=1000&limit=10", headers=auth_header
    )
    assert response.status_code == 200
    ...
    response = await client.get(
        "/api/v1/chats/?skip=0&limit=10000", headers=auth_header
    )
    assert response.status_code == 200
...
async def test_chat_name_filtering_edge_cases(client: AsyncClient, auth_header):
    ...
    for name in special_chars:
        await client.post(
            "/api/v1/chats/", headers=auth_header, json={"name": name, "member_ids": []}
        )
